classdef (StrictDefaults) CostasAmComplexPLL < matlab.System
    ...
    methods
        % Constructor
        function obj = CostasAmComplexPLL(varargin)
    ...
    methods(Access = protected)
        ...
        function [OutputFrame, FrequencyError] = stepImpl(obj, InputFrame)
            ...
            for n = 1:FrameSize
                ...
                MixedData = InputFrame(n) * exp(-1j*obj.NCOPhase);

                % синфазная ветвь
                InphaseBranch = real(MixedData);

                % квадратурная ветвь
                QuadratureBranch = imag(MixedData);

                % произведение ветвей
                LoopInput = InphaseBranch.*QuadratureBranch;
                    
                % петлевой фильтр
                kp_out = obj.kp * LoopInput;
                ki_out = obj.ki * LoopInput + obj.LoopFilterAcc;
