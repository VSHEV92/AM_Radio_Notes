classdef (StrictDefaults) SquareAmSignalPLL < matlab.System
    ...
    methods
        % Constructor
        function obj = SquareAmSignalPLL(varargin)
    ...
    methods(Access = protected)
        ...
        function setupImpl(obj)
            ...
            H = PLL_Highpass_FIR_Coeff();
            obj.HighpassFIR = dsp.FIRFilter(H.Numerator);
        end
        ...
        function [OutputFrame, FrequencyError] = stepImpl(obj, InputFrame)
            ...
            OutputFrame = zeros(FrameSize,1);
            FrequencyError = zeros(FrameSize,1);
            
            InputFrame = InputFrame.^2;
            InputFrame = obj.HighpassFIR(InputFrame);
            ...
            for n = 1:FrameSize
                ...
                % update nco phase
                obj.NCOPhase = obj.NCOPhase + loop_filter_out + 2*pi*(2*obj.CentralFrequency)*obj.Ts;
 
                % set outputs
                FrequencyError(n) = loop_filter_out / (2*pi) * obj.SampleFrequency; 
                OutputFrame(n) = cos(obj.NCOPhase/2);
            end
        end
